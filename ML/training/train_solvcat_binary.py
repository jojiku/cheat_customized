...
from solvgnn.model.model_GNN import solvcat_binary
...
def data_flip(solvdata):
    ...
    solvdata_flipped["inter_hb"] = solvdata["inter_hb"]
    solvdata_flipped["solv1_x"] = 1 - solvdata["solv1_x"]
    solvdata_flipped["gamma1"] = solvdata["gamma2"]
    solvdata_flipped["gamma2"] = solvdata["gamma1"]
    return solvdata_flipped


def train(epoch,train_loader,model,loss_fn1,loss_fn2,optimizer):
    stage = "train"
    model.train()
...
def validate(epoch,val_loader,model,loss_fn1,loss_fn2):
    stage = 'validate'
    batch_time = AccumulationMeter()
    loss_accum = AccumulationMeter()
    loss1_accum = AccumulationMeter()
...
def main():
    ...
    # read dataset file
    dataset_path = './solvgnn/data/output_binary_all.csv'
    ...
    for train_indices, valid_indices in kf.split(all_ind,tpsa_binary):
        ...
        # initialize model
        model = solvcat_binary(in_dim=74, hidden_dim=256, n_classes=2).cuda()
        model_arch = 'solvcat'
        ...
        for epoch in range(100):
            ...
            train_loss = train(epoch,train_loader,model,loss_fn1,loss_fn2,optimizer)
            train_loss_save.append(train_loss[0])
            train_loss1_save.append(train_loss[1])
            train_loss2_save.append(train_loss[2])
            val_loss = validate(epoch,val_loader,model,loss_fn1,loss_fn2)

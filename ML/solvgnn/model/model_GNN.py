        ...
        with g1.local_scope():
            with g2.local_scope():
                with g3.local_scope():
                    ...
                    hg1 = torch.cat((hg1, solv1x[:, None]), axis=1)
                    hg2 = torch.cat((hg2, solv2x[:, None]), axis=1)
                    hg3 = torch.cat((hg3, solv3x[:, None]), axis=1)
                    # hg1 = solv1x[:,None]*hg1
                    # hg2 = solv2x[:,None]*hg2
                    # hg3 = solv3x[:,None]*hg3
...
class solvgcn_binary(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        super(solvgcn_binary, self).__init__()
        self.conv1 = GraphConv(in_dim, hidden_dim)
        self.conv2 = GraphConv(hidden_dim, hidden_dim)
...
class solvgcn_ternary(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        super(solvgcn_ternary, self).__init__()
        self.conv1 = GraphConv(in_dim, hidden_dim)
        self.conv2 = GraphConv(hidden_dim, hidden_dim)
        self.global_conv1 = GraphConv(hidden_dim+1, hidden_dim)
        self.classify1 = nn.Linear(hidden_dim, hidden_dim)
        ...
        with g1.local_scope():
            with g2.local_scope():
                with g3.local_scope():
                    ...
                    hg = self.global_conv1(empty_solvsys,
                                        torch.cat((hg1,hg2,hg3),axis=0))
                    output = F.relu(self.classify1(F.relu(hg)))
                    output = F.relu(self.classify2(output))
                    output = self.classify3(output)                        
                    output = torch.cat(
                        (output[0:len(output)//3,:],
...
class solvcat_binary(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        super(solvcat_binary, self).__init__()
        self.conv1 = GraphConv(in_dim, hidden_dim)
        self.conv2 = GraphConv(hidden_dim, hidden_dim)
        self.classify1 = nn.Linear(hidden_dim*2 + 2, hidden_dim)
        self.classify2 = nn.Linear(hidden_dim, hidden_dim)
        self.classify3 = nn.Linear(hidden_dim, n_classes)

    def forward(self, solvdata):
        g1 = solvdata['g1']
        g2 = solvdata['g2']
        with g1.local_scope():
            with g2.local_scope():
                ...
                # hg1 = F.relu(self.classify1(hg1))
                # hg1 = F.relu(self.classify2(hg1))
                # hg1 = self.classify3(hg1)                        

                # hg2 = F.relu(self.classify1(hg2))
                # hg2 = F.relu(self.classify2(hg2))
                # hg2 = self.classify3(hg2)
                # output = torch.cat((hg1,hg2),axis=1)
                
                hg = torch.cat((hg1,hg2),axis=1)
                output = F.relu(self.classify1(hg))
                output = F.relu(self.classify2(output))
                output = self.classify3(output)                        
                return output
...
class solvcat_ternary(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        super(solvcat_ternary, self).__init__()
        self.conv1 = GraphConv(in_dim, hidden_dim)
        self.conv2 = GraphConv(hidden_dim, hidden_dim)
        self.classify1 = nn.Linear(hidden_dim*3 + 3, hidden_dim)
        self.classify2 = nn.Linear(hidden_dim, hidden_dim)
        self.classify3 = nn.Linear(hidden_dim, n_classes)
        ...
        with g1.local_scope():
            with g2.local_scope():
                with g3.local_scope():
                    ...
                    hg2 = torch.cat((hg2, solv2x[:, None]), axis=1)
                    hg3 = torch.cat((hg3, solv3x[:, None]), axis=1)
                    hg = torch.cat((hg1,hg2,hg3),axis=1)
                    output = F.relu(self.classify1(hg))
                    output = F.relu(self.classify2(output))
                    output = self.classify3(output)                        
        return output    
